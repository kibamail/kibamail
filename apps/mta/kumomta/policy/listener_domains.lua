...
kumo.on('get_listener_domain', function(domain, listener, conn_meta)
  ...
  if inbox_info.valid then
    ...
    conn_meta:set_meta('inbox_domain', 'true')
    conn_meta:set_meta('inbox_workspace_id', inbox_info.workspace_id or '')
    conn_meta:set_meta('inbox_sending_domain_id', inbox_info.sending_domain_id or '')
    return kumo.make_listener_domain {
  ...
  if bounce_info.valid then
    conn_meta:set_meta('bounce_workspace_id', bounce_info.workspace_id or '')
    conn_meta:set_meta('bounce_sending_domain', bounce_info.sending_domain or '')
    return kumo.make_listener_domain {

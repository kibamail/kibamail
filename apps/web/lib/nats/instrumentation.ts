...
const eventsProcessedCounter = meter.createCounter(
  "event_ingestor.events.processed",
  {
...
const eventsFailedCounter = meter.createCounter(
  "event_ingestor.events.failed",
  {
...
const suppressionsCreatedCounter = meter.createCounter(
  "event_ingestor.suppressions.created",
  {
...
const messagesAckedCounter = meter.createCounter(
  "event_ingestor.nats.messages.acked",
  {
...
const messagesNackCounter = meter.createCounter(
  "event_ingestor.nats.messages.nacked",
  {
...
const dbInsertDurationHistogram = meter.createHistogram(
  "event_ingestor.db.insert_duration",
  {
...
const suppressionDurationHistogram = meter.createHistogram(
  "event_ingestor.suppressions.duration",
  {
...
meter
  .createObservableGauge("event_ingestor.nats.pending_messages", {
...
meter
  .createObservableGauge("event_ingestor.nats.ack_pending", {
...
meter
  .createObservableGauge("event_ingestor.consumer.lag", {
...
export function recordEventsProcessed(
  count: number,
  eventType: string,
  workspaceId: string,
): void {
...
export function recordEventsFailed(
  count: number,
  eventType: string,
  errorType: string,
): void {
...
export function recordSuppressionCreated(
...
export function recordBatchDuration(
  durationMs: number,
  success: boolean,
): void {
...
export function recordDbInsertDuration(
  durationMs: number,
  eventCount: number,
): void {
  dbInsertDurationHistogram.record(durationMs, {
    event_count: String(eventCount),
  });
}

export function recordSuppressionDuration(
  durationMs: number,
  reason: string,
): void {
 ...
 */
export function startBatchSpan(
  batchSize: number,
  consumerName: string,
): BatchTraceContext {
 ...
 */
export function endBatchSpan(
  ctx: BatchTraceContext,
  success: boolean,
  error?: Error,
): void {
...
export function calculateBatchStats(
  events: Array<{ type: string; tenant_id: string }>,
): BatchStats {
  ...
  for (const event of events) {
    ...
    eventsByWorkspace[event.tenant_id] =
      (eventsByWorkspace[event.tenant_id] || 0) + 1;
  }

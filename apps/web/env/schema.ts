...
export const env = createEnv({
  ...
  server: {
     ...
     * MTA Injection URL
     *
     * The HTTP endpoint for injecting emails directly into the MTA.
     * This is the KumoMTA HTTP listener endpoint.
     *
     * @example "http://mta.cluster.kbmta.net:8000/inject"
     */
    MTA_INJECTION_URL: z
      .string()
      .url("MTA_INJECTION_URL must be a valid URL")
      .describe("MTA HTTP injection endpoint URL"),

    /**
     * MTA Injection Timeout (milliseconds)
     *
     * Maximum time to wait for a response from the MTA injection endpoint.
     * Increase this for large batches or slow networks.
     *
     * @default 30000
     * @example "30000"
     */
    MTA_INJECTION_TIMEOUT_MS: z
      .string()
      .transform((val) => parseInt(val, 10))
      .pipe(z.number().int().min(1000).max(300000))
      .default(30000)
      .describe("MTA injection request timeout in milliseconds"),

    /**
     * MTA Injection Retry Attempts
     *
     * Number of times to retry a failed injection request.
     * Uses exponential backoff between retries.
     *
     * @default 3
     * @example "3"
     */
    MTA_INJECTION_RETRY_ATTEMPTS: z
      .string()
      .transform((val) => parseInt(val, 10))
      .pipe(z.number().int().min(0).max(10))
      .default(3)
      .describe("Number of retry attempts for failed MTA injections"),

    /**
     * MTA Injection Batch Size
     *
     * Maximum number of emails to send in a single batch request to the MTA.
     * Larger batches are more efficient but may timeout.
     *
     * @default 100
     * @example "100"
     */
    MTA_INJECTION_BATCH_SIZE: z
      .string()
      .transform((val) => parseInt(val, 10))

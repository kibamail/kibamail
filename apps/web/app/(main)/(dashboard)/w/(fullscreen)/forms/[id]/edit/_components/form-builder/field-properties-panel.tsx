...
import { Xmark, Plus, Trash, Menu, Mail, Phone, Globe, Clock, MapPin, Text as TextIcon, Calendar, Hashtag } from "iconoir-react";
...
const STANDARD_PROPERTY_INFO: Record<string, { name: string; icon: React.ComponentType<{ className?: string }> }> = {
...
function getPropertyIcon(dbType: string): React.ComponentType<{ className?: string }> {
  switch (dbType) {
    case "DATE":
      return Calendar;
    case "NUMBER":
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-2 gap-1">
                {(["default", "secondary", "outline", "ghost"] as ButtonVariant[]).map((variant) => (
                  <button
                    key={variant}
                    type="button"
  ...
  return (
    <div className="w-[320px] h-full bg-kb-surface-secondary border-l border-kb-border-tertiary flex flex-col shrink-0">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5">
        ...
        {!PRESENTATION_FIELD_TYPES.includes(field.type as typeof PRESENTATION_FIELD_TYPES[number]) && (
          <div className="space-y-3">
            <h3 className="text-xs font-semibold text-kb-content-secondary uppercase tracking-wide">
              Contact Property
            </h3>
            ...
            <SelectField.Root
              ...
              onValueChange={(value) => {
                ...
                const isStandard = STANDARD_CONTACT_PROPERTIES.includes(value as typeof STANDARD_CONTACT_PROPERTIES[number]);
            ...
            {field.contactProperty && (
              <div className="flex items-center gap-2 px-3 py-2 rounded-md bg-kb-surface-primary border border-kb-border-tertiary">
                {field.contactProperty.type === "standard" ? (
                  (() => {
                    const Icon = STANDARD_PROPERTY_INFO[field.contactProperty.id]?.icon ?? TextIcon;
                    return <Icon className="w-4 h-4 text-kb-content-secondary" />;
                  })()
                ) : (
                  (() => {
                    const customProp = customProperties?.find((p) => p.id === field.contactProperty?.id);
                    const Icon = customProp ? getPropertyIcon(customProp.type) : TextIcon;
                    return <Icon className="w-4 h-4 text-kb-content-secondary" />;
                  })()
                )}
                ...
                <span className="text-xs text-kb-content-tertiary ml-auto">
                  {field.contactProperty.type === "standard" ? "Built-in" : "Custom"}
                </span>
              </div>
            )}
          </div>

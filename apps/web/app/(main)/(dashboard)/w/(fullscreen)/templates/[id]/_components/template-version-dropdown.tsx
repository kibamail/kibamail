...
interface TemplateVersionDropdownProps {
  currentTemplateId: string;
  currentVersion: number;
  currentStatus: EmailTemplateStatus;
  rootTemplateId: string;
  versions: TemplateVersionItem[];
  isLiveVersion: boolean;
}

function getStatusIcon(status: EmailTemplateStatus, isLive: boolean) {
  if (isLive) {
...
function getStatusLabel(status: EmailTemplateStatus, isLive: boolean) {
  if (isLive) {
    return "Live";
  }
  switch (status) {
...
function getStatusBadgeClasses(status: EmailTemplateStatus, isLive: boolean) {
  if (isLive) {
...
export function TemplateVersionDropdown({
  currentTemplateId,
  currentVersion,
  currentStatus,
  rootTemplateId,
  ...
  const createVersionMutation = useMutation({
    mutationFn: async () => {
      return internalApi.emailTemplates().createVersion(rootTemplateId);
    },
    onSuccess: (newVersion) => {
      toast("New draft created");
      queryClient.invalidateQueries({ queryKey: ["email-template"] });
      router.push(`/w/templates/${newVersion.id}`);
    },
    onError: (error: Error) => {
  ...
  const publishMutation = useMutation({
    mutationFn: async () => {
      return internalApi.emailTemplates().publish(currentTemplateId);
    },
    onSuccess: () => {
  ...
  function onVersionSelect(versionId: string) {
    if (versionId !== currentTemplateId) {
      router.push(`/w/templates/${versionId}`);
    }
  }
    ...
    <DropdownMenu.Root>
      ...
      <DropdownMenu.Content align="start" className="w-56">
        ...
        {versions.map((version) => {
          const isCurrentVersion = version.id === currentTemplateId;
          const isVersionLive =
